...
var filesToDelete 	= [".gitignore",".gitattributes", "LICENSE", "README.markdown", "_compileRelease.ahk", "ISSUE_TEMPLATE.md"];
...
function handleDownloadedFile() {
	extract(downloadDest + downloadFile, {dir: downloadDest}, function (err) {
		if(err) {
			...
			console.log("");
			handleExtractedFolders(false);
			handleExtractedFolders(true);
		}		
	})
}

function handleExtractedFolders(removeExe = false) {
	var folderSuffix = "";
	if (removeExe == false) {
		folderSuffix = "_exeIncluded";
	}
	
	// rename extracted folder and delete some files from it.
	var extractedFolderOld = downloadDest + "/" + repoProject + "-" + selectedBranch;
	var extractedFolderNew = downloadDest + "/" + repoProject + "-" + version + folderSuffix;
	fs.copySync(extractedFolderOld, extractedFolderNew)
	console.log(chalk.green(" Creating new zip archive: "));
	...
	if (removeExe) {
		console.log(" - " + "fallback.exe");
		fs.removeSync(extractedFolderNew + "/" + "fallback.exe");
	}
	
	console.log("");
	createZip(extractedFolderNew, folderSuffix);
}

function createZip(extractedFolderNew, folderSuffix) {
	var output = fs.createWriteStream(extractedFolderNew + ".zip");
	var archive = archiver('zip');

	output.on('close', function () {
		console.log(chalk.green(" Created archive: ") + repoProject + "-" + version + folderSuffix + ".zip with " + archive.pointer() + ' total bytes.');
...
function makeVersionSemverCompliant(version) {
	...
	if (v.length != version.length) {
		console.log(chalk.cyan(" Version not Semver compliant (leading zeros or other characters), removed them for comparison purposes only: " + version + " => " + v));
	}
